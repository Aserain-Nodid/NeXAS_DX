...
- 特点：目录扫描、树与卡片双视图、双击单文件操作、批量进度反馈、BSDX/BHE 引擎适配层

## 代码结构速览
- `src/main/java/com/giga/nexas/MainApplication.java`：JavaFX 启动入口
- `src/main/resources/fxml/MainView.fxml`：主界面布局
...
2. `ModeTreeController` 与 `BranchGridController` 共用数据模型，实现双向高亮；双击树叶节点等同于点击 `Run selected`
3. `ActionButtonController` 根据 `EngineType` 选择 `BinaryEngineAdapter`，支持单文件/类别/全局批处理，并记录失败详情
4. 进度条与标签实时反映任务进展，操作完成后在状态栏与日志输出汇总
5. 偏好设置（引擎、编码、路径锁定）通过 `Preferences` 保存并在启动时恢复

## 扩展指南
- 新增引擎：扩展 `EngineType`、实现 `BinaryEngineAdapter` 并在 `BinaryEngineFactory` 注册
- 新文件类别：在 `EngineType` 添加扩展名即可，扫描与视图会自动呈现
- 业务逻辑运行在单线程执行器中，涉及 UI 更新需通过 `Platform.runLater`
